...
G.Launch = function() {
    if (!INFRAME) {
        l('topBar').innerHTML =
            '<div><b>Cat Clicker</b>&trade; &copy; <a href="https://orteil.dashnet.org" target="_blank">Orteil</a>, 2021 - <a href="https://justinskittens.github.io/JKStudios" target="_blank">JKStudios</a></div>';
    }
    ...
    G._Init = function() {
        ...
        if (!G.local && !INFRAME) {
            window.cookieconsent_options = {
                "message": "This website uses cookies for ads and traffic analysis.",
                "dismiss": "Got it!";
            };
    ...
    G.button = function(obj) {
        ...
        if (obj.onclick || obj.tooltip) {
            G.pushCallback(function(id, obj) {
                return function() {
                    if (l(id)) {
                        if (typeof obj.tooltip === 'function') G.addTooltip(l(id), {
                            func: obj.tooltip
                        });
                        else G.addTooltip(l(id), {
                            text: obj.tooltip
                        });
    ...
    G.updateTextTimer = function(id, func, freq) {
        ...
        if (el) {
            ...
            if (freq < 0) {
                freq = -freq;
                delay = 100;
            } //a trick : the first update always occurs 100ms after being declared
            el.innerHTML = func();
            G.addCallbacks();
    ...
    G.selfUpdatingText = function(func, freq) {
        ...
        G.pushCallback(function(id, func, freq) {
            return function() {
                G.updateTextTimer(id, func, -freq);
            }
        }(id, func, freq));
    ...
    G.tooltipped = function(text, obj, style) {
        ...
        G.pushCallback(function(id, obj) {
            return function() {
                if (typeof obj === 'string') G.addTooltip(l(id), {
                    text: obj
                });
    ...
    G._Logic = function() {
        ...
        G.mouseDown = false;
        G.mouseUp = false;
        if (G.mouseMoved && G.mousePressed) G.draggedFrames++;
        else if (!G.mousePressed) G.draggedFrames = 0;
        G.mouseMoved = 0;
        G.Scroll = 0;
        G.clickL = 0;
        G.keysD = {};
...
if (!INFRAME) {
    ...
    window.addEventListener('message', function(e) {
        if (e.data.playerReady && !G.loadedPlayer) {
            G.loadedPlayer = true;
            l('player').contentWindow.postMessage({
                launch: true,
                loc: window.location.origin,
                vars: G.vars
            }, '*');
        } else if (e.data.editorReady) {} else if (e.data.code && !G.loadedEditor && l('editor')) {
            G.loadedEditor = true;
            l('editor').contentWindow.postMessage({
                code: e.data.code
            }, '*');
    ...
    if (!G.ready) {
        if (EDITOR) {
			...
			`;
            AddEvent(l('code'), 'input', function() {
                G.timeSinceTyped = Math.ceil(G.fps / 2);
            });
            G.Logic = function() {
                if (G.timeSinceTyped > 0) G.timeSinceTyped--;
                if (G.timeSinceTyped == 1) parent.postMessage({
                    refresh: l('code').value
                }, '*');
            }
            G._Init();
            parent.postMessage({
